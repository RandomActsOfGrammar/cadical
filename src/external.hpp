...
struct External {
  ...
  vector<ELit> extension;      // Solution reconstruction extension stack.
  ...
  //
  void push_clause_literal_on_extension_stack (ILit ilit);
  ...
  unsigned elit2ulit (ELit elit) const {
    assert (e_val(elit));
    assert (e_val(elit) != INT_MIN);
    const int idx = abs (e_val(elit)) - 1;
    assert (idx <= max_var);
    return 2u*idx + (e_val(elit) < 0);
  }
  ...
  ILit internalize (ELit);        // Translate external to internal literal.
  ...
  void check_assignment (int (External::*assignment) (ELit) const);
  ...
  //
  inline int sol (ELit elit) const {
    assert (solution);
    assert (e_val(elit) != INT_MIN);
    int eidx = abs (e_val(elit));
